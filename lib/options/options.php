 ...
 *
 */
if(!class_exists('Duplicate_Media_Checker_Options')) :
	abstract class Duplicate_Media_Checker_Options {
		...
		public function scripts() {
			...
			if(isset($this->fields['select'])) {
				wp_enqueue_style('select2-css', $this->selfpath.'/lib/select2/select2.min.css', array(), NULL);
				wp_enqueue_script('select2-js', $this->selfpath.'/lib/select2/select2.min.js', array('jquery'), FALSE, TRUE);
			}
			...
			if(has_action('enqueue_scripts_'.$this->setup['project_slug'])) {
				do_action('enqueue_scripts_'.$this->setup['project_slug']);
			}
		}
		...
		private function show_field_multi_select($id, $field, $subfield_id = NULL) {
			...
			echo '<select multiple="multiple" id="'.($subfield_id ? $subfield_id : $id).'" class="select2 '.$field['class'].'" name="'.$this->option_group.'['.$id.'][]'.($subfield_id ? '['.$subfield_id.']' : '').'" '.($field['disabled'] ? 'disabled="true"' : '').'>';
		...
		private function show_field_select($id, $field, $subfield_id = NULL) {
			...
			echo '<select id="'.($subfield_id ? $subfield_id : $id).'" class="select2 '.$field['class'].'" name="'.$this->option_group.'['.$id.']'.($subfield_id ? '['.$subfield_id.']' : '').'" '.($field['disabled'] ? 'disabled="true"' : '').'>';
